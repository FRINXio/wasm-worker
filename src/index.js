...
async function createTaskResult(
  ...
  console.info('createTaskResult updating task', {
    outputData,
    logs,
    reasonForIncompletion,
  });
...
async function checkAndRegister(wasmSuffix, healthCheckFn, executeFn) {
  ...
  if (!(await healthCheckFn())) {
    console.warn(`${wasmSuffix} healthcheck failed`);
  } else {
    console.info(
  ...
  registerWasmWorker(wasmSuffix, async (data, updater) => {
    ...
    try {
      ...
      console.error('Task has failed', {
        killed: e.killed,
        code: e.code,
        signal: e.signal,
        cmd: e.cmd,
      });
      console.info('Task has failed', {error: e});
...
async function init() {
  ...
  for (const [wasmSuffix, {healthCheckFn, executeFn}] of workers) {
    try {
      ...
      console.warn('Error in checkAndRegister of ' + wasmSuffix, {error});
    }
  }
}
